...
Ksub=[1,1000]; %Substrate spring constant in pN/nm
...
for ii = 1:length(Ksub)
    ...
    xsub=(Kc*sum(xc(ceng))/(Ksub(ii)+length(ceng)*Kc)); %calculate substrate position
    %vf=vu*(1-((Ksub(ii)*xsub)/(nm*Fm))); %claculate actin filament velocity
    %xc(ceng)=xc(ceng)+vf*dt; %claculate positions of engaged clutches (dt=0.005)
    ...
    for i=2:eventlimit
        ...
        xsub=(Kc*sum(xc(ceng))/(Ksub(ii)+length(ceng)*Kc)); %calculate substrate posisiton
        %vf=vu*(1-((Ksub(ii)*xsub)/(nm*Fm))); %claculate actin filament velocity
        %xc(ceng)=xc(ceng)+vf*dt; %claculate positions of engaged clutches
        xc(cdisen)=xsub; %calculate posisiton of disengaged clutches
        Fc=Kc*(xc-xsub); %calculate force on each clutch
